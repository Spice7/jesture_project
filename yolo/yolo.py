...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Parameter YAML (default: yolo/yolo_param.yaml; v3: yolo/v3.yaml).",
    )
    parser.add_argument(
        "--mode",
        choices=("train", "tune", "test", "predict", "val"),
        default="train",
    ...
    parser.add_argument("--device", help="Override device: cpu or 0 (default: YAML/auto)")
    return parser.parse_args()


def load_config(config_path: Path) -> tuple[dict[str, Any], Path]:
    config_path = resolve_local_path(SCRIPT_DIR.parent, config_path)
...
def resolve_dataset(data_yaml: str | Path) -> dict[str, Any]:
    """Resolve relative dataset paths from the source YAML, never Ultralytics' global datasets_dir."""
    source = Path(data_yaml).resolve()
    if not source.is_file():
        raise FileNotFoundError(f"학습용 data.yaml을 복사하세요: {source} (추론에는 불필요)")
    data = yaml.safe_load(source.read_text(encoding="utf-8"))
    if not isinstance(data, dict) or "names" not in data:
        raise ValueError(f"data.yaml에 클래스 names가 필요합니다: {source}")
    dataset_root = resolve_local_path(source.parent, data.get("path") or ".")
    data["path"] = str(dataset_root)
    for split in ("train", "val", "test"):
        value = data.get(split)
        if value is not None:
            if not isinstance(value, str):
                raise ValueError(f"{split}은 images 폴더 경로여야 합니다: {source}")
            data[split] = str(resolve_local_path(dataset_root, value))
    return data


@contextmanager
def prepared_dataset(data_yaml: str, splits: tuple[str, ...]):
    data = resolve_dataset(data_yaml)
    for split in splits:
        directory = Path(data.get(split, ""))
        if not data.get(split) or not directory.is_dir() or not any(
                p.suffix.lower() in {".jpg", ".jpeg", ".png", ".bmp", ".webp", ".tif", ".tiff"}
                for p in directory.rglob("*") if p.is_file()):
            raise FileNotFoundError(f"{split} 학습/평가 이미지를 넣으세요: {data.get(split, data_yaml)} "
                                    "(이미 학습된 모델의 추론에는 불필요)")
    # 원본 YAML은 변경하지 않는다. 절대 경로는 실행 중 임시 설정에만 존재한다.
    with tempfile.TemporaryDirectory(prefix="jesture-yolo-") as temporary:
        path = Path(temporary) / "data.yaml"
...
def main() -> None:
    ...
    if args.device is not None:
        for section in ("train", "validation", "test", "inference", "tune"):
            config.setdefault(section, {})["device"] = args.device
    ...
    if args.mode == "predict":
        ...
        project = str(resolve_local_path(config_dir, paths.get("project", "../runs/yolo")))
        run_predict(YOLO(trained_model), config, config_dir, project, args.source)
        return

    model_source, common_args = build_common_args(config, config_dir)
    if args.mode in ("test", "val"):
        paths = config["paths"]
        trained_model_value = paths.get("trained_model")
        if not trained_model_value:
    ...
    splits = (("test",) if args.mode == "test" else
              (config.get("validation", {}).get("split", "val"),) if args.mode == "val" else ("train", "val"))
    with prepared_dataset(common_args["data"], splits) as data_yaml:
        common_args["data"] = data_yaml
        model = YOLO(model_source)
        if args.mode == "test":
            run_test(model, config, common_args)
        elif args.mode == "val":
            val_args = {k: v for k, v in config.get("validation", {}).items() if k != "enabled"}
            metrics = model.val(**{**common_args, **val_args})
