...
def _pil_font(size):
    for name in ("malgunbd.ttf", "malgun.ttf", "arial.ttf"):
        p = Path(os.environ.get("WINDIR", "")) / "Fonts" / name
        if p.exists():
...
class App(ctk.CTk):
    def __init__(self, camera: int, live: bool, model: str | None, gestures: str | None,
                 static_model: str | None = None, device: str | None = None):
        super().__init__()
        ...
        try:
            if self.pipeline is None:
                ...
                self.pipeline = Pipeline(model_file=self.model_file, gestures_path=self.gestures_path,
                                         dry_run=not self._want_live, static_model=self.static_model,
                                         device=self.device)
            ...
            while not self._stop.is_set():
                ...
                if gate is not None and getattr(gate, "det", None) is not None:
                    prediction = gate.det.last_prediction
                    if prediction is not None:
                        frame = prediction.plot(img=frame.copy())
    ...
    def _refresh_gate(self, st=None):
        ...
        if st.gate_connected:
            ...
            self.gate_test.configure(state="disabled")
            self.gate_lbl.configure(text="게이트 오류 — 인식 중지", text_color="#e74c3c")
            self.gate_big.configure(text="오류로 닫힘", text_color="#e74c3c")
            self.gate_hint.configure(text=st.gate_error + " — 모델/장치를 확인한 뒤 다시 시작하세요.")
            self.pose_lbl.configure(text="")
            ...
            self.gate_hint.configure(text="설정에서 YOLO 게이트를 켜고 정적 모델을 준비한 뒤 다시 시작하세요."); self.pose_lbl.configure(text="")
        if st.pending_label:
...
def main():
    ...
    ap.add_argument("--static-model", default=None, help="정적 YOLO 모델 (기본 gestures.json의 gate.model)")
    ap.add_argument("--device", default=None, help="정적 YOLO 장치: cpu 또는 0 (기본 자동)")
    args = ap.parse_args()
    config.require_runtime_files(args.model, args.gestures, args.static_model)
    app = App(args.camera, args.live, args.model, args.gestures, args.static_model, args.device)
    app.mainloop()
