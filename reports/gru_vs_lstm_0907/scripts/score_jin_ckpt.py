...
def score(run_name, data_name, splits=("val", "test")):
    run_dir, data_dir = JIN / "artifacts" / run_name, JIN / "artifacts" / data_name
    if not (run_dir / "best_model.pt").exists():
        run_dir = JIN / run_name
    if not data_dir.exists():
        data_dir = JIN / data_name
    m, ck = load(run_dir)
...
if __name__ == "__main__":
    ...
    (SCR.parent / "raw" / "jin_ckpt_scores.json").write_text(json.dumps(out, ensure_ascii=False, indent=1), encoding="utf-8")
