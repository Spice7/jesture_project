...
SWIPE_MIN_DX = 0.5        # 스와이프: 손목 순이동(x+) 최소. 수집 데이터 최소 0.87. 09-07 실측 짧은 스와이프 0.49~0.58 이 걸려 0.6→0.5 (짧은 스와이프 증강과 함께)
...
# 09-07 오후, 6명 스냅 450개로 재보정: 사람마다 튕긴 뒤 엄지-중지 거리가 달라(p001 중앙값 0.61, p006 1.30) 0.8 은 p001 4/100 만 통과.
# pe>=0.5 rel>=0.3 idx>=1.0 → 스냅 425/450(94%, 사람별 최소 92%), no_gesture 누출 53/307, 주먹 4/497. 스냅 검사는 모델 뒤의 2차 방어.
SNAP_MIN_PINCH_END = 0.5    # 구간 뒤 1/3 에서 엄지끝-중지끝 최대 거리 (손바닥 단위). 주먹 p95 0.64
SNAP_MIN_RELEASE = 0.3      # (뒤 1/3 최대 핀치) - (구간 전체 최소 핀치). 6명 스냅 p5 0.35, no_gesture p50 0.08 / p95 0.74
