...
(순수 파이썬, `uv sync --locked` 로 같이 설치됨)로 다크 테마 사이드바 구성. 판정 로직은 `main.py` 와
...
```powershell
uv run python main.py            # 연습 모드(키 안 누름)로 시작. 시연 전 확인용
uv run python main.py --live     # 처음부터 실제 키 입력
uv run python main.py --camera 1 # 카메라 번호
```
...
| 파일 | 역할 |
|---|---|
| `main.py` | 화면(customtkinter). 카메라·인식은 워커 스레드, 화면은 30ms 마다 갱신. 1차 Tkinter 판은 커밋 0a1d4f0 |
