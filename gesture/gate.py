...
class StaticGate:
    def __init__(self, model_path: str | Path | None = None, confidence: float = 0.5, hold_seconds: float = 3.0,
        ...
        path = Path(model_path) if model_path else config.MODELS_DIR / "yolo_gate.pt"
        if not path.is_absolute():
            path = config.ROOT / path
        if not path.exists():
            raise FileNotFoundError(f"YOLO 게이트 모델이 없습니다: {path} (드라이브의 dataset_yolo/lkh/v2/best.pt 를 복사)")
        self.path = path
        if device is None:
